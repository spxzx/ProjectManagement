...
const AESKey = "suhvcthjkpdcvgtrfujnhgzw"
...
func EncryptInt64(id int64, keyText string) (cipherStr string, err error) {
	return Encrypt(strconv.FormatInt(id, 10), keyText)
}
...
func DecryptNoErr(cipherStr string, keyText string) int64 {
	str, _ := Decrypt(cipherStr, keyText)
	code, _ := strconv.ParseInt(str, 10, 64)
	return code
}

func EncryptNoErr(id int64, keyText string) (cipherStr string) {
	cipherStr, _ = Encrypt(strconv.FormatInt(id, 10), keyText)
	return
}
